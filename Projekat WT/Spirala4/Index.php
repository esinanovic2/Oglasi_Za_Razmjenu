...
<header>
	...
	<?php
		...
		{
			die("Neuspjelo povezivanje s bazom ". $conn->connect_error);
		}
		echo "<br>";
		echo "Uspješno spojeno s bazom";
		$sqlKorisnici="SELECT * FROM `korisnici`";
		$rezultat=$conn->query($sqlKorisnici);
		$usernames=null;
		$passwords=null;
		$i=0;
		while($red=$rezultat->fetch_assoc())
		{
			$usernames[$i]=$red['username'];
			$passwords[$i]=$red['password'];
			$i++;
		}
				
		$adminusername=provjera($usernames[0]);
		$adminpassword=provjera($passwords[0]);
	
	
		/*$korisnici=fopen("Korisnici.txt","r") or die("Datoteka se ne može otvoriti");
		...
		if(isset($_REQUEST['username'])&&($_REQUEST['username']!="")&&isset($_REQUEST['password'])&&($_REQUEST['password']!=""))
		{
			...
			if($adminusername==$username && sha1($password)==$adminpassword)
			{
...
<div id="novosti">
	<?php
		...
		$sqlVijesti="SELECT * FROM `vijesti`";
		$rezultatVijesti=$conn->query($sqlVijesti);
		$id; $naslov; $oglas; $url; $datum;

		$i=0;
		while($redVijest=$rezultatVijesti->fetch_assoc())
		{
			$id=$redVijest['id'];
			$naslov=$redVijest['naslov'];
			$oglas=$redVijest['oglas'];
			$url=$redVijest['url'];
			$phpdate = strtotime($redVijest['datum']);
			$datum = date( "M d, Y H:i:s", $phpdate );
			echo 
			"
				<section id='".$id."' class='novost'>
